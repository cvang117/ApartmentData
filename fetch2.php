...
if($_GET["query"] = '')
{
 ...
 $query = "
 SELECT * FROM evicted_tenant,lives_in
 WHERE Evicted_Tenant_Name IN (".$search_text.") 
 ORDER BY Evicted_Tenant_Name DESC";
 ...
 */
 //$query = "SELECT * FROM evicted_tenant
 //LEFT JOIN lives_in ON lives_in.Tenant_Apartment = evicted_tenant.Evicted_Tenant_Apartment";
 ...
 $query = "SELECT * FROM evicted_tenant ORDER BY Evicted_Tenant_Name DESC";
}
...
if($total_row > 0)
{
 foreach($result as $row)
 {
  $output .= '
  <tr>
   <td>'.$row["Evicted_Tenant_Name"].'</td>
   <td>'.$row["Evicted_Tenant_SSN"].'</td>
   <td>'.$row["Evicted_Tenant_Phone"].'</td>
   <td>'.$row["Evicted_Tenant_Apartment"].'</td>
   <td>'.$row["Action"].'</td>
  </tr>
  ';
 }
